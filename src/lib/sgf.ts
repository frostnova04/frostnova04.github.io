...
/**
 * Minimal SGF (Smart Game Format) support for the Weiqi/Kifu viewer.
 * Server-only (uses fs). Runs at build time to list & parse games in /sgf.
 */
...
function extractMainlineNodes(sgf: string): string[] {
  ...
  const traverse = (start: number): number => {
    ...
    while (j < n) {
      ...
      if (c === '[') {
        // copy the bracketed value verbatim so any '(' ')' inside it is ignored
        buf += c;
        j++;
        while (j < n && sgf[j] !== ']') {
          buf += sgf[j];
...
export function getSgfGame(id: string): SgfGame | null {
  // id must be a plain filename stem (no path separators / extensions)
